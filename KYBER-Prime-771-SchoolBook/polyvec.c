...
/*************************************************
* Name:        polyvec_matrix_mul
*
* Description: School Book multiplication between matrix(K*K) and vectors(K). 
**************************************************/
void polyvec_matrix_mul(polyvec *t, polyvec mat[KYBER_K], polyvec *s) {
  unsigned int i;

  for(i = 0; i < KYBER_K; i++)
    polyvec_mul(&t->vec[i], &mat[i], s);
}
...
* Name:        polyvec_mul
*
* Description: School Book multiplication between vectors(K) and vectors(K).
**************************************************/
void polyvec_mul(poly *r, polyvec *a, polyvec *b) {
    unsigned int i;
    poly t;

    poly_mul(r, &a->vec[0], &b->vec[0]);
    for(i=1;i<KYBER_K;i++) {
      poly_mul(&t, &a->vec[i], &b->vec[i]);
      poly_add(r, r, &t);
    }
}
/*************************************************
