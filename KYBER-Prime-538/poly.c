...
**************************************************/
void poly_compress_4(uint8_t r[KYBER_POLYCOMPRESSEDBYTES_CV], poly *a) {
    unsigned int i, j;
    uint8_t t[4];
    poly_caddq(a);
    ...
    for (i = 0; i < KYBER_N / 4; i++) {
        for (j = 0; j < 4; j++) {
            t[j] = ((((uint32_t)a->coeffs[4*i+j] << 4) + q/2) / q) & 15;
        }
    ...
    t[0] = ((((uint32_t)a->coeffs[268] << 4) + q/2) / q) & 15;
    r[0] = t[0];
}
...
void poly_compress_11(uint8_t r[KYBER_POLYCOMPRESSEDBYTES_CU], poly *a) {
    ...
    t[0] = ((((uint32_t)a->coeffs[264] << 11) + q/2) / q) & 0x7ff;
    t[1] = ((((uint32_t)a->coeffs[265] << 11) + q/2) / q) & 0x7ff;
    t[2] = ((((uint32_t)a->coeffs[266] << 11) + q/2) / q) & 0x7ff;
    t[3] = ((((uint32_t)a->coeffs[267] << 11) + q/2) / q) & 0x7ff;
    t[4] = ((((uint32_t)a->coeffs[268] << 11) + q/2) / q) & 0x7ff;
...
**************************************************/
void poly_decompress_4(poly *r, const uint8_t a[KYBER_POLYCOMPRESSEDBYTES_CV]) {
    unsigned int i,j;
    uint8_t t[4];
    ...
    for (i = 0; i < KYBER_N / 4; i++) {
        ...
        for (j = 0; j < 4; j++)
            r->coeffs[4 * i + j] = ((uint32_t) (t[j] & 15) * q + 8) >> 4;
    }
    t[0] = (a[0] >> 0);
    r->coeffs[268] = ((uint32_t) (t[0] & 15) * q + 8) >> 4;
}
...
void poly_decompress_11(poly *r, const uint8_t a[KYBER_POLYCOMPRESSEDBYTES_CU]) {
    ...
    t[3] = (a[4] >> 1) | ((uint16_t) a[5] << 7);
    t[4] = (a[5] >> 4) | ((uint16_t) a[6] << 4);
    r->coeffs[264] = ((uint32_t) (t[0] & 0x7FF) * q + 1024) >> 11;
    r->coeffs[265] = ((uint32_t) (t[1] & 0x7FF) * q + 1024) >> 11;
    r->coeffs[266] = ((uint32_t) (t[2] & 0x7FF) * q + 1024) >> 11;
    r->coeffs[267] = ((uint32_t) (t[3] & 0x7FF) * q + 1024) >> 11;
    r->coeffs[268] = ((uint32_t) (t[4] & 0x7FF) * q + 1024) >> 11;
}
...
void polyeta_frombytes(poly *r, const uint8_t a[KYBER_SECRETBYTES]) {
    ...
    for (i = 0; i < KYBER_N / 4; i++) {
        ...
        for (j = 0; j < 4; j++)
            r->coeffs[4 * i + j] = KYBER_ETA1 - t[j];
    }
    t[0] = (a[0] >> 0) & 15;
    r->coeffs[268] = KYBER_ETA1 - t[0];
}
...
void poly_rec(uint8_t m[KYBER_INDCPA_MSGBYTES], const uint8_t c2[KYBER_POLYCOMPRESSEDBYTES_CV], const poly *su) {
    ...
    for (i = 0; i < 32; i++) {
        m[i] = 0;
        for (j = 0; j < 8; j++) {
              tmp = ((int32_t)c2_coeffs[8 * i + j] + 4) * q - ((int32_t)su->coeffs[8 * i + j] << 4);
              tmp = (tmp * 19237) >> 30;
              //tmp = tmp / 55816 - ((uint64_t)tmp >> 63);
              tmp = tmp & 1;
              m[i] |= tmp << j;
        }
    }
